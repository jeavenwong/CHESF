...
//using Menge::Agents::GlobalPlan;
using MengeVis::Viewer::GLViewer;
using MengeVis::Viewer::OpenGLViewer;
...
SimulatorDB simDB;
std::string viewerType="openGL";
//std::string viewerType = "ogre";
...
int simMain(SimulatorDBEntry * dbEntry, const std::string & behaveFile,
	...
	SimulatorInterface * sim = dbEntry->getSimulator(agentCount, TIME_STEP, SUB_STEPS,
		...
		scbVersion, VERBOSE);
//设置各模块插件信息
	sim->setGlobalPlan(globalplan);
	sim->setTacticalPlan(tacticalplan);
	sim->setOperationPlan(operationplan);
	for each (EnvironmentalChange * var in environmentalChangeSet)
	{
		sim->addEnvironmentalChange(var);
	}
	if (viewer_name == "ogre"){
		viewerType = viewer_name;
	}
	else{ 
		viewerType = "openGL"; 
	}
...
//初始化人群信息
	...
	if (visualize) {
		...
		if (viewCfgFile == "") {
			...
			// TODO: Error handling
			if (viewCfg.readXML(viewCfgFile)) { //解析View（视角）相关的xml文件，参数保存在viewCfg中
				if (VERBOSE) {
		...
		GLViewer view(viewCfg);//可能是在补充一些默认配置
		...
		system->populateScene(scene);//这里的sim已经初始化完成了，数据已经加载了
		scene->addSystem(system);

		if (!viewerType.compare("openGL")){
			OpenGLViewer view(viewCfg);//可能是在补充一些默认配置
			view.setDumpPath(dumpPath);
			if (!view.initViewer(viewTitle)) {  //初始化Simulation 窗口（可视化引擎）
		...
		else if (!viewerType.compare("ogre")){
			GLViewer view(viewCfg);//可能是在补充一些默认配置
			view.setDumpPath(dumpPath);
	...
	std::cout << "...Finished\n";
	std::cout << "Sim```	2ulation time: " << dbEntry->simDuration() << "\n";
	logger << Logger::INFO_MSG << "Simulation time: " << dbEntry->simDuration() << "\n";

	return 0;
}

int mengemain(int argc, char* argv[]) {
	...
	CorePluginEngine plugins(&simDB);//初始化model（ORCA , PedVO）,以及注册各种工厂类(BFSM,agent相关)
	logger.line();
	...
	std::string model = globaldata_model;//根据Qt中选择的模型,不再从xml文件中获取
	std::cerr << "The specified model is : " << model << "\n";
	...
	//核心起点，加载
	std::string behaviorStr = projSpec.getBehavior();
	std::string sceneStr = projSpec.getScene();
	std::string outputNameStr = projSpec.getOutputName();
	std::string SCBVersionStr = projSpec.getSCBVersion();

	int result = simMain(simDBEntry, behaviorStr, sceneStr, outputNameStr, SCBVersionStr, useVis, viewCfgFile, dumpPath);
