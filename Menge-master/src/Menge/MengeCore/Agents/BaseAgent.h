...
namespace Menge {
	...
	namespace Agents {
		...
		public:
			 ...
			 */
			Math::Vector2	_pos;
			Math::Vector2	_nextGoal;//战术层 计算的 每一时刻的目标点
			Math::Vector2	_nextPrefSpeed;// 战术层 计算的 目标点与当前位置决定的 理想速度（方向，要单位化后乘速度）
			
			std::vector<Menge::Agents::Map *> _maps;
			std::vector<bool> environmentFamiliaritySet;//对map是否熟悉
			std::vector<Math::Vector2> _globalGoals;
			int currentMapID;
			int nextMapID;
			bool _needRePlan=false;//是否需要全局规划
			/*!
			...
			//保存每个agent的团队成员ID（就是在_agents中的下标）
			std::vector<const BaseAgent*> _groupAgents;
			/*!
			...
			void insertAgentGroup(const BaseAgent* agent);
			//
			virtual size_t getGoalInterface() const { return -1; }
			virtual  Math::Vector2 computeNextGoal() { return  Math::Vector2(0,0); }
			virtual  Math::Vector2 computeNextPrefSpeed() const { return  Math::Vector2(0, 0); }
			/*!
