...
namespace Menge {
	...
	namespace Agents {
		...
		public:
			...
			//返回值表示 是否成功做了规划
			virtual bool AgentGlobalPlan(int);
			virtual bool AgentTacticalPlan(int);
			virtual bool AgentOperationPlan(int);
			/*!
			...
			void setGlobalPlan(GlobalPlan * globalPlan);
			void setTacticalPlan(TacticalPlan * tacticalPlan);
			void addEnvironmentalChange(EnvironmentalChange * eChange);
			std::vector<EnvironmentalChange *> getEnvironmentalChange();
			void setOperationPlan(OperationPlan * op);
			//void addVisNode(MengeVis::SceneGraph::GLNode * eChange);
			/*!
      ...
      const SpatialQuery& getSpatialQuery() const { return *_spatialQuery; }
			...
			bool hasGlobalPlan() const { return _globalPlan != 0x0; }
			bool hasTacticalPlan() const { return _tacticalPlan != 0x0; }
			bool hasOperationPlan() const { return _operationPlan != 0x0; }
			/*!
		...
		protected:
			...
			GlobalPlan	* _globalPlan =0x0;
			TacticalPlan * _tacticalPlan = 0x0;
			OperationPlan * _operationPlan = 0x0;
			//std::vector<MengeVis::SceneGraph::GLNode*> _visNodes; 
		    std::vector<EnvironmentalChange *> _environmentalChangeSet;
			/*!
