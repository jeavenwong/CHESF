...
namespace Menge {
	...
	public:
		...
		*/
		...
		const int getNodeCount() { return _navMesh->getNodeCount(); }
		/*!
		 ...
		 *	ResourceManager uses it to load and instantiate NavMeshLocalizer instances.
		 该功能与ResourceManager一起使用。 
		 这就是为什么它返回一个指针，指向的不是一个NavMeshLocalizer，而是一个资源。
		 ResourceManager使用它来加载和实例化NavMeshLocalizer实例。
		 *
