...
namespace Menge {
	 ...
	 *
	  这是一个用于促进解析XML规范的实用程序类。
      它用于可以完全通过单个标签中的XML属性完全定义的元素。 它允许处理这些类型的规范而不用明确地处理XML。
	 ...
	 *
	   每个元素工厂都有一个内置的属性集（称为_attrSet）。
	   在工厂的构造函数中，只需调用属性集即可提供每个所需属性的详细信息。
	   属性集返回每个新属性的唯一标识符。 存储这些属性后，您将使用它们来提取解析的值。
	 ...
	 *	the instance.
	 在setFromXML函数中，首先调用父类的函数实现。
	 如果出现解析错误，则会自动抛出异常。 
	 你不需要抓住它。 该框架将抓住例外，并作出相应的回应。
	 假设没有例外，只需使用存储的标识符从属性集中提取值并将其设置为实例即可。
	 */
