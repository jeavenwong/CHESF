...
//#include "MengeCore/AgtHelbing/HelbingDBEntry.h"
...
namespace Menge {
	namespace PluginEngine {
		...
		CorePluginEngine::CorePluginEngine( SimulatorDB	* simDB ) : 
			...
			//registerModelDBEntry(new Helbing::DBEntry());
			...
			Agents::MapSetDB::initialize();
			Agents::ProfileSelectorDB::initialize();
			Agents::StateSelectorDB::initialize();

			Agents::GlobalPlanDB::initialize();
			Agents::EnvironmentalChangeDB::initialize();
			Agents::TacticalPlanDB::initialize();
			Agents::OperationPlanDB::initialize();
