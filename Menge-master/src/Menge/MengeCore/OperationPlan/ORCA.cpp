...
namespace Menge {
	...
	namespace Agents {
		...
		const float OperationORCAPlan::TAU = 2.5f;
		///< The default value for tau obstacles (the time horizon w.r.t. obstacles).
		const float OperationORCAPlan::TAU_OBST = 0.15f;
		const std::string OperationORCAPlan::NAME = "orca";
		float TIME_STEP;
		////////////////////////////////////////////////////////////////////////////
		//			Implementation of 
		////////////////////////////////////////////////////////////////////////////
		OperationORCAPlan::OperationORCAPlan(){
			_planName = "ORCA";
			_timeHorizon = TAU;
		...
		void OperationORCAPlan::obstacleLine(size_t obstNbrID, const float invTau, bool flip) {
			const Menge::Agents::Obstacle* obst =_baseAgent->_nearObstacles[obstNbrID].obstacle;
			...
			const Vector2 relativePosition1 = P0 - _baseAgent->_pos;
			const Vector2 relativePosition2 = P1 - _baseAgent->_pos;
			...
			*/
			...
			for (size_t j = 0; j < _orcaLines.size(); ++j) {
				if (det(invTau * relativePosition1 - _orcaLines[j]._point,
					_orcaLines[j]._direction) - invTau * _baseAgent->_radius >=
					-Menge::EPS && det(invTau * relativePosition2 - _orcaLines[j]._point,
						_orcaLines[j]._direction) - invTau * _baseAgent->_radius >= -Menge::EPS) {
					alreadyCovered = true;
					break;
				}
			}
			...
			const float radiusSq = sqr(_baseAgent->_radius);
			...
			if (s < 0 && distSqLine <= radiusSq) {
				...
				*/
				...
				leftLegDirection = Vector2(relativePosition1.x() * leg1 -
					relativePosition1.y() * _baseAgent->_radius,
					relativePosition1.x() * _baseAgent->_radius +
				...
				*/
				...
				leftLegDirection = Vector2(relativePosition2.x() * leg2 -
					relativePosition2.y() * _baseAgent->_radius,
					relativePosition2.x() * _baseAgent->_radius +
			...
			*/
			...
			const float t = obstaclesSame ?
				0.5f : ((_baseAgent->_vel - leftCutoff) * (cutoffVec / absSq(cutoffVec)));
			const float tLeft = ((_baseAgent->_vel - leftCutoff) * leftLegDirection);
			const float tRight = ((_baseAgent->_vel - rightCutoff) * rightLegDirection);

			if ((t < 0.0f && tLeft < 0.0f) || (obstaclesSame && tLeft < 0.0f && tRight < 0.0f)) {
				/* Project on left cut-off circle. */
				const Vector2 unitW = norm(_baseAgent->_vel - leftCutoff);
				line._direction = Vector2(unitW.y(), -unitW.x());
				line._point = leftCutoff + _baseAgent->_radius * invTau * unitW;
				_orcaLines.push_back(line);
				return;
			}
			else if (t > 1.0f && tRight < 0.0f) {
				/* Project on right cut-off circle. */
				const Vector2 unitW = norm(_baseAgent->_vel - rightCutoff);
				line._direction = Vector2(unitW.y(), -unitW.x());
				line._point = rightCutoff + _baseAgent->_radius * invTau * unitW;
				_orcaLines.push_back(line);
				return;
			}
			...
			*/
			const float distSqCutoff = ((t < 0.0f || t > 1.0f || obstaclesSame) ?
				...
				absSq(_baseAgent->_vel - (leftCutoff + t * cutoffVec)));
			const float distSqLeft = ((tLeft < 0.0f) ?
				std::numeric_limits<float>::infinity() :
				absSq(_baseAgent->_vel - (leftCutoff + tLeft * leftLegDirection)));
			const float distSqRight = ((tRight < 0.0f) ?
				std::numeric_limits<float>::infinity() :
				absSq(_baseAgent->_vel - (rightCutoff + tRight * rightLegDirection)));

			if (distSqCutoff <= distSqLeft && distSqCutoff <= distSqRight) {
				/* Project on cut-off line. */
				line._direction = -obstDir;
				line._point = leftCutoff + _baseAgent->_radius * invTau * Vector2(-line._direction.y(),
					line._direction.x());
				_orcaLines.push_back(line);
			}
				...
				if (!isRightLegForeign) {
					...
					line._point = rightCutoff + _baseAgent->_radius * invTau * Vector2(-line._direction.y(),
						line._direction.x());
					_orcaLines.push_back(line);
				}
			}
		...
		size_t OperationORCAPlan::computeORCALines() {
			_orcaLines.clear();

			const float invTimeHorizonObst = 1.0f / _timeHorizonObst;

			/* Create obstacle ORCA lines. */
			for (size_t i = 0; i <_baseAgent->_nearObstacles.size(); ++i) {

				const Menge::Agents::Obstacle* obst = _baseAgent->_nearObstacles[i].obstacle;
				const Vector2 P0 = obst->getP0();
				const Vector2 P1 = obst->getP1();
				const bool agtOnRight = leftOf(P0, P1, _baseAgent->_pos) < 0.f;
			...
			for (size_t i = 0; i < _baseAgent->_nearAgents.size(); ++i) {
				const BaseAgent* const other = static_cast<const BaseAgent *>(_baseAgent->_nearAgents[i].agent);

				const Vector2 relativePosition = other->_pos - _baseAgent->_pos;
				const Vector2 relativeVelocity = _baseAgent->_vel - other->_vel;

				const float distSq = absSq(relativePosition);
				const float combinedRadius = _baseAgent->_radius + other->_radius;
				...
				if (distSq > combinedRadiusSq) {
					...
					line._point = _baseAgent->_vel + 0.5f * u;
				}
				else {
					...
					line._point = _baseAgent->_vel + coopWeight * u;
				}
		...
		void OperationORCAPlan::computeNewVelocity(float timesetp) {
			TIME_STEP = timesetp;
			Vector2 velPref(_baseAgent->_velPref.getPreferredVel());
			//Vector2 velPref = _nextPrefSpeed;		
			const size_t numObstLines = computeORCALines();

			size_t lineFail = linearProgram2(_orcaLines, _baseAgent->_maxSpeed, velPref, false, _baseAgent->_velNew);

			if (lineFail < _orcaLines.size()) {
				linearProgram3(_orcaLines, numObstLines, lineFail, _baseAgent->_maxSpeed, _baseAgent->_velNew);
			}
		}
		...
		void  OperationORCAPlan::initAgentInfo() const {
			
				return;	
		}

		OperationORCAPlanFactory::OperationORCAPlanFactory() : OperationPlanFactory() {
			_nameID = _attrSet.addStringAttribute( "type", true, "" );
		}

		////////////////////////////////////////////////////////////////////////////

		bool OperationORCAPlanFactory::setFromXML(OperationPlan * sel, TiXmlElement * node,
													const std::string & specFldr ) const {
			OperationORCAPlan * op = dynamic_cast< OperationORCAPlan * >( sel );
