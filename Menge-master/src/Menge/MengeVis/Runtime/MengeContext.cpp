...
namespace MengeVis {
	namespace Runtime {
		...
		bool MengeContext::selectGL( const SceneGraph::GLScene * scene,
		...
		void MengeContext::drawUIGL( int vWidth, int vHeight, bool select ) {
			if ( !select && _selected != 0x0 ) {
				...
				writeToScreen( _stateContext.getUIText( "   " ),
							   SceneGraph::TextWriter::RIGHT_TOP, 15, 10.f, 10.f );
				drawIDTyping();
			}
		}
		...
		void MengeContext::cancelIDTyping() {
			_state = DEFAULT_ST;
		}
