...
typedef struct agentOgre1{
	Ogre::Vector3 pos0;//agent 只有一个点
	Ogre::Vector3 pos1;//obstacle 有两个点
	std::vector<Ogre::Vector3> pos3;//ground有一系列点
	int ID;//agent Or obstacl
};
...
namespace MengeVis {
	...
	namespace SceneGraph {
		...
		public:
			 ...
			 */
			void drawGL( const GLCamera & camera, const GLLightVector & lights, int vWidth,
						 int vHeight );
			void drawGLOgre(const GLCamera & camera, std::vector<agentOgre1>& NodePos);
			bool firsttime;
