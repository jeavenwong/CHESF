...
namespace Menge {
	...
	namespace Agents {
		...
		void  TacticalPlanFollowPlugin::initAgentInfo(std::vector<Agents::Map *> _maps,
			std::vector<BaseAgent *> agents
		) const {
			_tacticalAgents.clear();
			//遍历_agents 保存所有agent的id信息，给每个Agent设置团队影响力
			for (size_t i = 0; i < agents.size(); i++) {
				FollowAgent* tpa = new FollowAgent();
				...
				_tacticalAgents.insert(std::pair<int, FollowAgent*>(agents[i]->_id, tpa));
	    ...
	    Menge::Math::Vector2 TacticalPlanFollowPlugin::computeNextGoal(){
			/*
			默认返回 第一个 全局目标点
			...
			*/
			...
			map<int, FollowAgent*>::iterator iter;
		...
		TacticalPlanFollowPluginFactory::TacticalPlanFollowPluginFactory() : TacticalPlanFactory() {
			_nameID = _attrSet.addStringAttribute( "type", true, "" );
		}

		////////////////////////////////////////////////////////////////////////////

		bool TacticalPlanFollowPluginFactory::setFromXML(TacticalPlan * sel, TiXmlElement * node,
													const std::string & specFldr ) const {
			TacticalPlanFollowPlugin * dGP = dynamic_cast< TacticalPlanFollowPlugin * >( sel );
		...
		size_t TacticalPlanFollowPlugin::getGoalInterface() const {
		...
		size_t TacticalPlanFollowPlugin::getGoal() {
		...
		Menge::Math::Vector2 TacticalPlanFollowPlugin::computeNextPrefSpeed() {
...
extern "C" {
		...
		TacticalPlanFollowPlugin_API	Menge::Agents::TacticalPlanFactory * Export(void) {

		return new Menge::Agents::TacticalPlanFollowPluginFactory();
